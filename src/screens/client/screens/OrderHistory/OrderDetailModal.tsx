...
export type OrderEstado = 'entregado' | 'noEntregado' | 'transito' | 'pendiente' | 'espera' | 'recogida' | 'devuelto';
...
export default function OrderDetailModal({ visible, order, onClose }: OrderDetailModalProps) {
  ...
  const isDevuelto:    boolean = order.estado === 'devuelto';
  ...
  const getBadge = (): BadgeConfig => {
    ...
    if (isDevuelto)    return { style: { backgroundColor: '#EDE9FE', borderRadius: 999, paddingHorizontal: 12, paddingVertical: 6 }, textStyle: { color: '#7C3AED', fontWeight: '700' as const, fontSize: 13 }, label: '↩️ Devuelto' };
  ...
  const descargarGuia = async (): Promise<void> => {
    ...
    <body>
      ...
      ` : ''}
      <div class="footer">
        GEOSERVIT EXPRESS · ${order.id} · ${new Date().toLocaleString('es-CO')}
      </div>
    </body>
    </html>
    `;
        ...
        <View style={styles.sheet}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.statusRow}>
              ...
              {(isEntregado || isNoEntregado || isDevuelto) && order.fechaEntrega && (
